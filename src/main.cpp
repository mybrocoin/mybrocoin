...
uint256 hashGenesisBlock("0xdc122cc3e0171c36a0c2f866e24614d5cebd621e35f6483e70a21ceb40ae3272");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // MyBroCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "MyBroCoin Signed Message:\n";
...
{
    // MyBroCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // MyBroCoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 24 * 60 * 60; // MyBroCoin: 1 days
static const int64 nTargetSpacing = 120; // MyBroCoin: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // MyBroCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // MyBroCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // MyBroCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xc749097ac5cf4d613468fa76aed77aca71765e5fa1e9962a040474b7ad13dcf5");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
       ...
       txNew.vout[0].scriptPubKey = CScript() << 0x0 << OP_CHECKSIG; // a privkey for that 'vanity' pubkey would be interesting ;)
        ...
        block.nVersion = 1;
        block.nTime    = 1389319436; //epochtime
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 4845551;

        if (fTestNet)
        {
            block.nTime    = 1389300737;
            block.nNonce   = 296388;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x670831a47429604a3e11faa32fa2c4561d7c7f82dbdd6d3764a3cfbbaba1f39d"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // MyBroCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MyBroCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MyBroCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MyBroCoinMiner(CWallet *pwallet)
{
    printf("MyBroCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("mybrocoin-miner");
    ...
    try { loop {
        ...
        printf("Running MyBroCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MyBroCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MyBroCoinMiner, pwallet));
}
