...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // MarioBrosCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "MarioBrosCoin Signed Message:\n";
...
{
    // MarioBrosCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // MarioBrosCoin: 840k blocks in ~4 years
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // MarioBrosCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // MarioBrosCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // MarioBrosCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // MarioBrosCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MarioBrosCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MarioBrosCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MarioBrosCoinMiner(CWallet *pwallet)
{
    printf("MarioBrosCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("mariobroscoin-miner");
    ...
    try { loop {
        ...
        printf("Running MarioBrosCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MarioBrosCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MarioBrosCoinMiner, pwallet));
}
