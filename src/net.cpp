...
bool GetMyExternalIP(CNetAddr& ipRet)
{
    ...
    for (int nHost = 1; nHost <= 2; nHost++)
    {
        ...
        if (nHost == 1)
        {
            addrConnect = CService("216.146.39.70", 80); // checkip.dyndns.org
...
CNode* ConnectNode(CAddress addrConnect, const char *pszDest)
{
    ...
    /// debug print
    printf("trying connection %s %i lastseen=%.1fhrs\n",
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "MarioBrosCoin " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
    {"mariobroscoin.com", "dnsseed.mariobroscoin.com"},
    {NULL, NULL}
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. MarioBrosCoin is probably already running."), addrBind.ToString().c_str());
        else
